...
import { Plus, Search, Pencil, Eye, Trash2, Receipt, ChevronDown, FileText, FilePlus, IndianRupee } from 'lucide-react'
...
export default function Invoices() {
  ...
  const handleSavePayment = (amount: number) => {
    ...
    updateInvoice(paymentInvoice.id, { paidAmount: amount, paidDate: amount > 0 ? new Date().toISOString() : undefined })
    ...
    <div className="space-y-4">
      ...
      <TableCard>
        <Table>
          ...
          <Tbody>
            {filtered.length === 0 ? (
              <TEmpty colSpan={7} icon={<Receipt className="w-10 h-10" />} message="No invoices yet" />
            ) : filtered.map((inv, i) => (
              <Tr key={inv.id} clickable onClick={() => navigate(`/invoices/${inv.id}/edit`)}>
                ...
                <Td align="right">
                  {!!inv.paidAmount ? (
                    <div>
                      <span className="font-bold text-green-600">{formatCurrency(inv.paidAmount)}</span>
                      {inv.paidAmount < inv.grandTotal && (
                        <div className="text-[11px] text-amber-600 font-semibold mt-0.5">Due {formatCurrency(inv.grandTotal - inv.paidAmount)}</div>
                      )}
                    </div>
                  ) : <span className="text-gray-300">—</span>}
                </Td>
                <Td align="center" onClick={e => e.stopPropagation()}>
                  <div className="flex items-center justify-center gap-1.5">
                    ...
                    <button onClick={e => { e.stopPropagation(); setPaymentInvoice(inv) }} className="w-8 h-8 flex items-center justify-center border border-gray-200 text-green-500 hover:bg-green-50 hover:border-green-200 rounded-lg transition-colors" title="Add Payment"><IndianRupee className="w-4 h-4" /></button>
      ...
      {paymentInvoice && (
        <PaymentModal
          documentNumber={paymentInvoice.number}
          grandTotal={paymentInvoice.grandTotal}
          currentAmount={paymentInvoice.paidAmount}
